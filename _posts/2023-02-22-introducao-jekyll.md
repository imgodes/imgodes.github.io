...
A partir daqui você pode pular para instação, ou ler sequencialmente. Acho que o melhor jeito é intercalar, leia rapidamente e vá seguindo com a prática, caso se perca em algum momento, basta retornar.
...
- Tudo que vem após os 3 ultimos traços `---` será considerado conteúdo da postagem. Esse conteúdo será formatado pelo seguinte arquivo `default.html`.

![htmlsample](layouthtml.png){: .center }
_Exemplo de arquivo html usado para definicão de layout_
...
Faça o clone do seu reposítorio da maneira que preferir. Dois exemplos:
```shell
gh repo clone SEU-USERNAME/SEU-USERNAME.github.io
git clone https://github.com/SEU-USERNAME/SEU-USERNAME.github.io.git
```

Com o repositório, podemos seguir com a instação dos gems necessários:
```shell
bundle install 
```
